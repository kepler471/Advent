...
Determine the number of trees you would encounter if, for each of the following slopes, you start at the top-left corner and slide the map all the way to the bottom:
...
let rec slide (lst : string list) (slope : int*int) (count : int64) ((x,y) : int*int) =
  match lst with
    | [] -> count
    | [""] -> count
    | h::t ->
...
let main _ =
  ...
  printfn "Part 2 (test). %A" ( slopes
                         |> List.map (fun s -> slide example s 0L (0,0))
                         |> List.reduce (*) )

  printfn "Part 2. %A" ( slopes
                         |> List.map (fun s -> slide input s 0L (0,0))
                         |> List.reduce (*) )
