...
let diagnosticReport = 
    [ for i in input 
        do yield [for c in i do yield float(c.ToString())]
    ]

let rec parseReport lines acc =
    match lines with
    | [] -> acc
    | h :: t -> parseReport t (List.map2 (+) h acc)
...
// Can this be done with a map2 or some kind of pairwise sequence?
let binToDec binlist = 
    let rec loop binlist pow acc =
        match binlist with
        | [] -> acc
        | h :: t -> loop t (pow + 1) ((h <<< pow) + acc)
    loop (List.rev binlist) 0 0

let oxygen (matrix: float list list) =
    let rec reduce (matrix: float list list) col =
        match matrix with
        | [row] -> row
        | m -> 
...
let co2 (matrix: float list list) =
    let rec reduce (matrix: float list list) col =
        match matrix with
        | [row] -> row
        | m -> 
