...
let autocompleteScores = 
    input 
    |> Array.map (parse (fun x -> 
        if x = ' ' then true else false))     
    |> Array.filter (fun (x, _) -> x)
    |> Array.map (fun (_, stack) -> 
        stack 
        |> List.map (mapScore >> uint64) 
        |> List.reduce calcScore)
    |> Array.sort

printfn "--- Day 10: Syntax Scoring ---"
printfn "Part One: -> %A" (input |> Array.map (parse mapScore) |> Array.map fst |> Array.sum)
printfn "Part Two: -> %A" (autocompleteScores.GetValue (autocompleteScores.Length / 2))