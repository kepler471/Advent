...
# how such a system got into a volcano, but you don't have time to complain; your device produces a report (your
# puzzle input) of each valve's flow rate if it were opened (in pressure per minute) and the tunnels you could use to
...
# pressure release of 28 * 13 = 364. Then, you could spend your third minute moving to valve CC and your fourth
# minute opening it, providing an additional 26 minutes of eventual pressure release at a flow rate of 2, or 52 total
...
# --- Part Two ---
#
# You're worried that even with an optimal approach, the pressure released won't be enough. What if you got one of
# the elephants to help you?
#
...
from utils import flatten, read_input, swap, lmap
import numpy as np


def parse(line): return list(flatten([word.strip(",").strip(";").split("=") for word in line.split()]))


lines = read_input(16, 2022, test=True).splitlines()
routes = {x[1]: x[10:] for x in [parse(line) for line in lines]}
rates = {x[1]: int(x[5]) for x in [parse(line) for line in lines] if int(x[5]) != 0}
ij = {r: n for n, r in enumerate(routes)}  # map node to matrix index

dim = len(set(routes.keys()))
M = np.matrix(np.zeros([dim, dim]))  # build adjacency matrix list[list[bool]]
M[:, :] = np.inf
...
# Find the shortest path for all pairs (APSP), Floyd-Warshall
for k, _ in enumerate(ij):
    for i, _ in enumerate(ij):
        for j, _ in enumerate(ij):
            M[i, j] = min(M[i, j], M[i, k] + M[k, j])

np.fill_diagonal(M, 0)


# shout out juanlopes: https://github.com/juanplopes/advent-of-code-2022/blob/main/day16.py
def visit(a: str, open_valves: set[str], visits: dict[tuple[str]: int], flow: int, time: int):
    visits[tuple(sorted(open_valves))] = max(visits.get((tuple(sorted(open_valves))), 0), flow)

    for b in rates:
        newtime = time - M[ij[a], ij[b]] - 1
        if b in open_valves or newtime <= 0:
            continue
        visit(b, open_valves | {b}, visits, flow + newtime * rates[b], newtime)
    return visits
...
y = visit("AA", set(), {}, 0, 30)
print(max(x.values()))
total2 = max(v1 + v2 for k1, v1 in y.items()
             for k2, v2 in y.items() if not k1 & k2)
